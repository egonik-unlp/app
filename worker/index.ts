...
async function resolveEndpoint(env: Env, uri: string, state: CoreState): Promise<Resolved> {
  ...
  return {
    ...
    display: {
      ...
      release_year: cold.release_year ?? null,
    },
    exact: false,
    snappedId: r.snap_id,
...
function requestedNode(r: Resolved, pos: number[], opened = false): any {
  ...
  return {
    ...
    release_year: r.display.release_year ?? null,
...
function requestedFromNode(n: any, pos: number[]): any {
  return {
    ...
    release_year: n.release_year ?? null,
...
function openApiSpec(origin: string) {
  const trackNode = {
    ...
    properties: {
      ...
      genre: { type: "string" },
      release_year: { type: "integer", nullable: true, description: "Release year of the track's album, when known; null otherwise." },
